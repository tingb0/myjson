...
static const char* lept_parse_hex4(const char* p, unsigned* u,int *flag) {
    ...
    *flag = 0;
    ...
    unsigned  high_s = strtol(p, NULL, 16);
    if (high_s >= 0xD800 && high_s <= 0xDBFF) {//高代理项合法
        if (*temp == '\\') {
            temp++;
            if (*temp == 'u') {
                temp++;
                ...
                unsigned low_s = strtol(temp, NULL, 16);
                if (low_s >= 0xDC00 && low_s <= 0xDFFF) { //低代理项合法
                    *u = 0x10000 + (high_s - 0xD800) * 0x400 + (low_s - 0xDC00);
                    return temp1;
                }
                else { *flag = 1; return NULL; }
            }
...
static void lept_encode_utf8(lept_context* c, unsigned u) {
    ...
    if (u >= 0x0000 && u <= 0x007F) {
        ...
        PUTC(c, (0xC0 | ((u >> 6) & 0xFF))); /* 0xA0 = 11000000 */
